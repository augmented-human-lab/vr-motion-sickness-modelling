...
def worker(dataset):
    ...
    with open("main_log_3.csv", 'a') as main_log_file:
        ...
        with zipfile.ZipFile(dataset_abs_path, mode='r') as archive:
            ...
            for item in sorted(archive.namelist(), reverse=True):
                ...
                if "_gaze/data" in item:
                    exp_res1,gaze=extract_gaze(archive, item, dataset_name)
                ...
                if "VRLOG" in item:
                    extract_VRlog(archive, item, dataset_n, exp_res1, output_dir)
...
def main():
    ...
    tasks= ['/data/VR_NET/zipped/5/88_1_Metdonalds.zip']
